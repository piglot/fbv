...
int ym_jpeg_id(char *name)
{
    ...
    read(fd,id,10);
    close(fd);
    if(id[6]=='J' && id[7]=='F' && id[8]=='I' && id[9]=='F') return(1);//第7-10字节为“JFIF”，识别APPO标记
    if(id[0]==0xff && id[1]==0xd8 && id[2]==0xff) return(1);//前三个字节为0xff,0xd8,0xff
    return(0);
}
...
int ym_jpeg_load(char *filename,unsigned char *buffer, unsigned char ** alpha, int x,int y)
{
    ...
    ciptr=&cinfo;
    if(!(ym=fopen(filename,"rb"))) return(YM_ERROR_FILE);
...
int ym_jpeg_getsize(char *filename,int *x,int *y)
{
    struct jpeg_decompress_struct cinfo;
    struct jpeg_decompress_struct *ciptr;
    struct r_jpeg_error_mgr emgr;
