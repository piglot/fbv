...
int ym_bmp_id(char *name)//读取位图头信息，判断是否为"BM" 
{
	int fd;
	char id[2];
	
	fd = open(name, O_RDONLY);//为参数fd赋值为函数，以只读的形式打开文件
	if (fd == -1) 
	{
		return(0);
	}
	
	read(fd, id, 2);//把参数fd 所指的文件传送2个字节到数组id中
	close(fd);//关闭文件
	if ( id[0]=='B' && id[1]=='M' )//确认是否为BMP图像
	{
		return(1);
	}
	return(0);
...
void fetch_pallete(int fd, struct color pallete[], int count)//获取调色板信息
{
	...
	lseek(fd, BMP_COLOR_OFFSET, SEEK_SET);  //参数28为新的读写位置                                     
	for (i=0; i<count; i++) {
		read(fd, buff, 4);
...
//由上面可知：从bfOffBits开始4个字节每一行扫描的字节数
	if (lseek(fd, BMP_BPP_OFFSET, SEEK_SET) == -1)//从文件28处开始读写
	 {
		return(YM_ERROR_FORMAT);
	 }
	read(fd, buff, 2);
...
int ym_bmp_getsize(char *name,int *x,int *y) //获取bmp图片的大小信息
{
	int fd;
	...
	read(fd, size, 4);//把参数fd所指的文件传送4个字节到size中
